...
resource "aws_api_gateway_deployment" "analytics_api_deployment" {
  depends_on = [
    ...
    aws_api_gateway_integration.analytics_stats_integration
  ]
  rest_api_id = aws_api_gateway_rest_api.analytics_api.id
...
resource "aws_api_gateway_stage" "analytics_api_stage" {
  ...
  description   = "Env for analytics API"
}
...
# ----------------------------------------------------------------------------
# API Gateway Resource: /analytics-stats
# ----------------------------------------------------------------------------
resource "aws_api_gateway_resource" "analytics_stats_resource" {
  rest_api_id = aws_api_gateway_rest_api.analytics_api.id
  parent_id   = aws_api_gateway_rest_api.analytics_api.root_resource_id
  path_part   = "analytics-stats"
}

resource "aws_api_gateway_method" "analytics_stats_get_method" {
  rest_api_id   = aws_api_gateway_rest_api.analytics_api.id
  resource_id   = aws_api_gateway_resource.analytics_stats_resource.id
  http_method   = "GET"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "analytics_stats_integration" {
  rest_api_id             = aws_api_gateway_rest_api.analytics_api.id
  resource_id             = aws_api_gateway_resource.analytics_stats_resource.id
  http_method             = aws_api_gateway_method.analytics_stats_get_method.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.analytics_stats_lambda.invoke_arn
}

resource "aws_lambda_permission" "allow_api_gateway_invoke_analytics_stats" {
  statement_id  = "AllowExecutionFromAPIGatewayAnalyticsStats"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.analytics_stats_lambda.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.analytics_api.execution_arn}/*/*"
}
