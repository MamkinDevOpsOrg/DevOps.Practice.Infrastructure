...
resource "aws_api_gateway_deployment" "analytics_api_deployment" {
  depends_on  = [aws_api_gateway_integration.analytics_integration]
  rest_api_id = aws_api_gateway_rest_api.analytics_api.id
...
resource "aws_api_gateway_method" "analytics_get_method" {
  rest_api_id   = aws_api_gateway_rest_api.analytics_api.id
  resource_id   = aws_api_gateway_resource.analytics_resource.id # ВАЖНО: используем уже существующий ресурс analytics!
  http_method   = "GET"
  authorization = "NONE"
}
